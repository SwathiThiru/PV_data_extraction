...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    for table in tables:
        ...
        #table_name = [obj for obj in objects if iob(obj['bbox'], table['bbox'] >= 0.5)]
    ...
    for column_num, column in enumerate(columns):
        for row_num, row in enumerate(rows):
            ...
            cell = {'bbox': list(cell_rect), 'column_nums': [column_num], 'row_nums': [row_num],
                    'column header': header, 'table name': table_name}
...
def output_result(key, val, args, img, img_file, pdf_files):
    if key == 'objects':
        if args.verbose:
            print(val)
...
def merge_tables(pdf_files_folder, output_folder, pdf_files):
    for pdf_file in pdf_files:
        ...
        for filename in os.listdir(pdf_files_folder):
            if filename.startswith(base_name) and filename.endswith('.xlsx'):
                # Read the Excel file
                df = pd.read_excel(os.path.join(pdf_files_folder, filename))
...
def main():
    ...
    img_input_folder = image_dir + "/../detectionOutput"
    words_output_folder = image_dir + "/../words"
    ...
    for count, img_file in enumerate(img_files):
        ...
        if args.mode == 'extract':
            ...
            #    excel_file = os.path.join(args.out_dir, file.replace(".pdf", ".xlsx"))
            #    wb = Workbook()
            #    wb.save(excel_file)
            ...
            for table_idx, extracted_table in enumerate(extracted_tables):
                for key, val in extracted_table.items():
                    output_result(key, val, args, extracted_table['image'],
                                  img_file.replace('.jpg', '_{}.jpg'.format(table_idx)), pdf_files)
