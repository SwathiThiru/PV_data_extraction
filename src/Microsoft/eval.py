...
import src.Microsoft.postprocess as postprocess
import src.Microsoft.grits as grits
from src.Microsoft.grits import grits_con, grits_top, grits_loc


structure_class_names = [
    'table', 'table column', 'table row', 'table column header',
    'table projected row header', 'table spanning cell', 'no object',
    'table row header', 'table projected column header', 'table name'
]
...
def compute_metrics(mode, true_bboxes, true_labels, true_scores, true_cells,
    ...
    metrics['acc_con'] = metrics['grits_con']
...
def box_cxcywh_to_xyxy(x):
    # Check for empty bounding box tensor
    if x.numel() == 0:  # Check if there are no elements
        return torch.empty((0, 4))  # Return an empty tensor with shape (0, 4)

    # Ensure x is at least 2D with correct shape
    if x.dim() == 1:
        x = x.unsqueeze(0)  # Convert (4,) to (1, 4) if needed
    if x.size(1) != 4:
        raise ValueError(f"Expected bounding box tensor shape (N, 4), but got {x.shape}")
...
def rescale_bboxes(out_bbox, size):
    if out_bbox.numel() == 0:  # If no bounding boxes are present
        return torch.empty((0, 4))  # Return an empty tensor with shape (0, 4)
...
def eval_tsr_sample(target, pred_logits, pred_bboxes, mode):
    ...
    print({"file": img_words_filepath, "metrics": metrics})
