...
import src.Microsoft.table_datasets as TD
from src.Microsoft.table_datasets import PDFTablesDataset
from src.Microsoft.eval import eval_coco
...
def get_data(args):
    ...
    training_data_load_path = os.path.join(args.data_root_dir, "train")
    #training_data_load_path = args.data_root_dir + "/train"

    validation_data_load_path = os.path.join(args.data_root_dir, "val")
    #validation_data_load_path = args.data_root_dir + "/val"
    if args.mode == "train":
        ...
        dataset_val = PDFTablesDataset(validation_data_load_path,
        ...
        dataset_test = PDFTablesDataset(test_path,
                                        get_transform(args.data_type, "val"),
                                        do_crop=False,
...
def get_model(args, device, checkpoint):
    """
    ...
    if args.model_load_path:
        ...
        model.class_embed = nn.Linear(model.class_embed.out_features, 10)
        #num_classes = 9
        ...
        #pretrained_dict = {k: v for k, v in loaded_state_dict.items() if k not in model_state_dict.keys() or 'classifier' in k}
        pretrained_dict = {k: v for k, v in checkpoint.items() if 'class_embed' not in k}

        """if 'model' in checkpoint:
            pretrained_dict = {k: v for k, v in checkpoint['model'].items() if 'class_embed' not in k}
        elif 'state_dict' in checkpoint:
            pretrained_dict = {k: v for k, v in checkpoint['state_dict'].items() if 'class_embed' not in k}
        else:
            #pretrained_dict = {k: v for k, v in checkpoint.items() if 'class_embed' not in k}
            pretrained_dict = {k: v for k, v in loaded_state_dict.items() if k not in model_state_dict.keys() or 'classifier' in k}"""
    ...
    #criterion = nn.CrossEntropyLoss(weight=torch.ones(num_classes).to(device))
    else:
...
def train(args, model, criterion, postprocessors, device):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch+1) % args.checkpoint_freq == 0:
            model_save_path_epoch = os.path.join(output_directory, 'model_' + args.data_type + str(epoch+1) + '.pth')
...
def load_model(model_load_path):
    if model_load_path is None:
        ...
        return None  # Return None to indicate no checkpoint was loaded
...
def update_model_for_new_classes(model, num_classes_new):
    """
    Update the model's final layer to reflect the new number of classes.
    This assumes `class_embed` is the layer that needs adjustment.
    """
    in_features = model.class_embed.in_features  # Get the number of input features
    model.class_embed = nn.Linear(in_features, num_classes_new)

    # Initialize the new layer's weights
    nn.init.xavier_uniform_(model.class_embed.weight)
    model.class_embed.bias.data.fill_(0.01)
...
def main():
    ...
    checkpoint = load_model(args.model_load_path)
    #checkpoint = torch.load(args.model_load_path, map_location='cpu')
    model, criterion, postprocessors = get_model(args, device, checkpoint)

    # only for finetuning the model
    model = update_model_for_new_classes(model, args.num_classes)
    #criterion = nn.CrossEntropyLoss(weight=torch.ones(config_args["num_classes"]).to(device))
