...
class Datasheet():
    ...
    def extract_electrical_props(
        ...
        if elec_table is None:
            print("warning: No Electrical Characteristics Table Found")
            return None
        ...
        try:
            ...
            print("warning: Exception occured during table axis detection")

        if axis is None:
            print("warning: Cannot Determine Table Axis, Cannot Continue")
            return None
    ...
    def _find_rows_llm(
        ...
        Parameters:
            ...
            patterns: These are prompts descriptions that will guide the LLM for extraction.
    ...
    def _call_llm_for_extraction(self, prompt: str) -> str:
        """
        Function to call LLM (e.g., GPT-4) with a given prompt and extract data.
    ...
    def extract_final_props(
        ...
        """
        This function will attempt to extract the final properties
        from the raw tables that are the present in this datasheet. This is
        the master function that will make use of other internal class
        functions in order to keep things organised.
        ...
        if len(final_table) == 0:
            final_table_table = None
        elif len(final_table) == 1:
            final_table_table = final_table[0]
        else:
            max_len = 0
            selected_table = None
    ...
    def extract_temp_props(
        ...
        if temp_table is None:
            print("warning: No Temperature Coefficient Table Found")
            return None
    ...
    def extract_pack_props(
        ...
        """
        This function will attempt to extract the packaging information
        from the raw tables that are the present in this datasheet. This is
        the master function that will make use of other internal class
        functions in order to keep things organised.
        ...
        Parameters:
            patterns:
                Theses are regex patterns that will be used to detect the
                right columns or rows that contain the required packaging
                information.
        """
    ...
    def extract_mech_props(
        ...
        if mech_table is not None:
            """# Convert the table data into a string or plain text format
            temp_list = mech_table.raw_df.values.tolist()
            temp_list = list(chain.from_iterable(temp_list))  # Flatten the table data
            table_string = ' '.join(map(str, temp_list))  # Create a string representation

            # Construct a prompt for the LLM
            prompt = (
                "Given the following mechanical properties table, "
                "please extract the length and width dimensions:\n\n"
                f"{table_string}\n\n"
                "Extract the mechanical dimensions for length and width, in meters or millimeters."
            )

            # Call the LLM with the table data and the prompt
            response = llm.extract(prompt)  # Replace with actual LLM call

            # Handle the LLM response and parse the extracted dimensions
            extracted_values = self._parse_llm_response_for_mech(response)

            if extracted_values:
                length, width = extracted_values.get('length'), extracted_values.get('width')
            else:
                print("LLM could not extract dimensions.")"""
            temp_list = mech_table.raw_df.values.tolist()
            ...
            print("warning: No Mech Table Found")
