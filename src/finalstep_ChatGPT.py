...
class Datasheet():
    ...
    def __init__(
        ...
        # Dictionary type for ectracted electrical properties in NMOT conditions
    ...
    def extract_electrical_props(
        ...
        if axis == "vertical":
            ...
            found_columns = self._find_columns_llm(
                table=elec_table,
                patterns=patterns
            )
            ...
            found_rows = self._find_rows_llm(
                table=elec_table,
                patterns=patterns
            )
    ...
    def extract_mech_props(
        ...
        if mech_table is not None:
            ...
            if extracted_value:
                length, width = extracted_value.get('length'), extracted_value.get('width')
            else:
    ...
    def _call_llm_for_column_extraction(self, prompt: str) -> str:
        ...
        try:
            """response = openai.Completion.create(
                engine="gpt-4o-mini",  # You can also use 'gpt-3.5-turbo'
                prompt=prompt,
                max_tokens=100,  # Adjust token limit based on expected response size
                temperature=0  # Lower temperature for more deterministic responses
            )"""
            response = openai.ChatCompletion.create(
                model="gpt-4o-mini",  # Use the appropriate model, such as gpt-3.5-turbo or gpt-4
                messages=[
                    {"role": "system",
                     "content": "You are an expert in extracting data from solarmodule datasheets."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=300,  # You can adjust the token limit
                temperature=0.5
            )
    ...
    def _call_llm_for_extraction(self, prompt: str) -> str:
        ...
        try:
            ...
            response = openai.ChatCompletion.create(
                ...
                messages=[
                    {"role": "system",
                     "content": "You are an expert in extracting data from solar module data sheets."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=300,  # You can adjust the token limit
                temperature=0.5
