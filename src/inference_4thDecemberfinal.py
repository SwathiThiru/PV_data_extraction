...
from finalstep_4thDecemberfinal import Datasheet
...
def rescale_bboxes(out_bbox, size):
    """
        Rescales bounding box coordinates from a normalized format
...
def align_headers(headers, rows):
    ...
    at least 50% of the height of the row.
        ...
        try:
            ...
            with Image.open(image_path).convert('RGB') as img:
                ...
                with open(output_json_path, 'w') as json_file:
                    ...
                    print("OCR file created at path : ", output_json_path)
...
def objects_to_structures(objects, tokens, class_thresholds):
    ...
    This include resolving conflicts/overlaps, and ensuring the boxes meet certain alignment
    conditions.
...
class TableExtractionPipeline(object):
    ...
    def fs_file(
        ...
        with open(self.model_dir + "/patterns_3rdDecember.yaml", "r", encoding='utf-8') as stream:
            try:
...
def get_list_of_files_with_ext(
    ...
    for file in sorted(listdir(path_to_folder)):

        # Skip the hidden files
        # In linux and macOS, the hidden files start with '.'
...
def main():
    ...
    if args.mode == 'detect' or args.mode == 'extract':
        # for Docker
        words_dir = "/app/inferences/detectionwords"
        ...
        # for Docker
        words_dir = "/app/inferences/words"
    ...
    for count, img_file in enumerate(img_files):
        ...
        if not words_dir is None:
            ...
            with open(tokens_path, 'r') as f:
                ...
                # list of tokens in a reading order
    ...
    if args.mode == 'detect':
        ...
        print("Successfully detected all tables.")

    if args.mode == 'recognize':
        #print("PDF file dierctory is ", pdf_files_dir)
        ...
        print("Successfully recognized the structure of all tables.")

    if args.mode == 'extract':
        pipe.perform_final_step(args.in_dir, args.in_dir)
        pipe.save_to_excel(path=os.path.join(args.in_dir, "extracted_tables.xlsx"))
        print("Successfully extracted the relevant data from of all tables.")
